...
typename Client<SpecT>::SharedPtr create_client_impl(  //创建客户端封装对象，并启用日志记录。
...
typename Service<SpecT>::SharedPtr create_service_impl(  //创建服务端封装对象，并启用日志记录。
...
typename Publisher<SpecT>::SharedPtr create_publisher_impl(NodeT * node)  //创建发布者封装对象。
{
  // This function is a wrapper for the following.
...
typename Subscription<SpecT>::SharedPtr create_subscription_impl(   //创建订阅者封装对象
  NodeT * node, CallbackT && callback)
{
  ...
  auto subscription = node->template create_subscription<typename SpecT::Message>(
    SpecT::name, get_qos<SpecT>(), std::forward<CallbackT>(callback));  //根据规范类型 SpecT 创建 QoS 配置。
