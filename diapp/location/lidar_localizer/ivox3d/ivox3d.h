...
class IVox {
   public:
    using KeyType = Eigen::Matrix<int, dim, 1>; //键类型
    using PtType = Eigen::Matrix<float, dim, 1>;//点类型
   ...
   private:
    ...
    Options options_;
    std::unordered_map<KeyType, typename std::list<std::pair<KeyType, NodeType>>::iterator, hash_vec<dim>> grids_map_; // voxel hash map
